...
import { buildFullRatings } from "../engine";
import { Landmark } from "../types/engine";
...
const angle = (a: Landmark[number], b: Landmark[number]): number => {
...
export function computeGeometry3DScores(
  ...
  const eye =
    100 * (1 - ASYM(pts, range(33, 133), range(263, 362)) / M);
  ...
  const scores = buildFullRatings(cal);
